"""掃除判定エンジン — brainの新しい頭脳(掃除ファースト)。

出発点の問題: 「20-30個のウィンドウが片付かないまま溜まる」。
各セッションを「閉じてOK / 引き継ぎ要 / ユーザー判断 / 稼働中 / 再開可能」に分類し、
ウィンドウを減らすための具体的な掃除アクションを提示する。

純関数(latest.jsonのsession recordを受けて判定を返す)。CLI・MCP・ダッシュボード・
脳プロンプトの全入口から同じ判定を使う。
"""
