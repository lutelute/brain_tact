...
4. IDLE(放置)への標準メッセージ例: 「定時巡回です。現在の進捗を3行で要約し、残作業があれば自律的に続けてください。区切りではテスト・実行確認を通し、論理単位でコミットしてください。判断が必要な点だけ箇条書きで止めておいてください」— 改善には**検証可能な締め**(テスト/実行確認/Web・UIならヘッドレスブラウザでの実操作確認/コミット)を含める。具体的な実装内容は指示せず、何をどう検証するかはセッションに選ばせる(賢い相手の判断を尊重)
...
3. 【止まりを動かす】完了報告・放置で止まっているセッション(closeable/resumable)には act_send で「完了と思っても、自分で一度見直して、価値ある改善が残っていれば続けて」と促す。**何を直すかはセッションに任せ、あなたが粗を逐一挙げない**(賢い相手の判断を尊重)。完了報告も「お疲れさま」で終わらせず、自己評価を続けさせる
